...
// props
defineOptions({ name: 'KitFormECPoint' });
...
<template>
  <div>
    ...
    <KitFormBigint
      v-model="key.Q.x"
      :title="`${title}.x (${getBIBits(key.Q.x)} byte)`"
    />
    <KitFormBigint
      v-model="key.Q.y"
      :title="`${title}.y (${getBIBits(key.Q.x)} byte)`"
    />
    <KitCollapse v-if="fold" class="my-2 bg-base-300">
      <template #header>
        Point Compression
      </template>
      <KitFormToggle v-model="key.Q.isInfinity" :title="`${title}.isInfinity`" />
      <!-- Compression -->
